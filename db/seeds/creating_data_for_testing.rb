...
user_names.map do |name, file_name|
...
puts 'Creating tasks for all users ...'
...
puts 'Inviting users to tasks ...'

User.all.each do |user|
  task_creator = User.where.not(id: user.id).sample
  TaskInvitation.create(task_id: Task.where(user_id: task_creator.id).first.id, user_id: user.id)
end
